...
import com.deepblue.disinfect.utils.Duration;
import com.deepblue.disinfect.utils.Instant;
...
public class TransformativeImageView2 extends AppCompatImageView {
    ...
    public TransformativeImageView2(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TransformativeImageView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setGraffiti(boolean graffiti) {
        this.isGraffiti = graffiti;
    }

    public void setPoint(boolean setPoint) {
        this.isPoint = setPoint;
    }
    ...
    private void obtainAttrs(AttributeSet attrs) {
        ...
        TypedArray typedArray = getContext()
                .obtainStyledAttributes(attrs, R.styleable.TransformativeImageView2);
        mMaxScaleFactor = typedArray.getFloat(
                R.styleable.TransformativeImageView2_max_scale2, MAX_SCALE_FACTOR);
        mMinScaleFactor = typedArray.getFloat(
                R.styleable.TransformativeImageView2_min_scale2, UNSPECIFIED_SCALE_FACTOR);
        mRevertDuration = typedArray.getInteger(
                R.styleable.TransformativeImageView2_revert_duration2, DEFAULT_REVERT_DURATION);
        mOpenScaleRevert = typedArray.getBoolean(
                R.styleable.TransformativeImageView2_open_scale_revert, false);
        mOpenRotateRevert = typedArray.getBoolean(
                R.styleable.TransformativeImageView2_open_rotate_revert, false);
        mOpenTranslateRevert = typedArray.getBoolean(
                R.styleable.TransformativeImageView2_open_translate_revert, false);
        mOpenAnimator = typedArray.getBoolean(
                R.styleable.TransformativeImageView2_open_animator, true);
        mScaleBy = typedArray.getInt(
                R.styleable.TransformativeImageView2_scale_center2, SCALE_BY_FINGER_MID_POINT);
        typedArray.recycle();
    }
...
//        setLayerType(View.LAYER_TYPE_SOFTWARE, null);
    ...
    private void initImgPositionAndSize() {
        ...
        mMatrix.getValues(mInitMatrixValue);

        if (listener != null) {
            listener.onScaleChanged(mScaleFactor);
        }
    }
    ...
    public boolean canSingleFingerMove = true;//可单指移动

    @Override
    public void setImageBitmap(Bitmap bm) {
        super.setImageBitmap(bm);
        if (getWidth() > 0 && getHeight() > 0) {
            float maxScale = Math.max(getWidth() / bm.getWidth(), getHeight() / bm.getHeight());
            int scale = (int) (maxScale + 1.5);
            setMaxScaleFactor(Math.max(2, scale));
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (isPoint || isGraffiti) {
                    if (event.getPointerCount() == 1) {
                        pressDownPointF.x = event.getX();
                        pressDownPointF.y = event.getY();
                        if (listener != null) {
                            listener.onTouched(pressDownPointF);
                        }
                    }
                } else {
...
//                    mCanDrag = false;
                        pressDownPointF.x = event.getX();
                        pressDownPointF.y = event.getY();
                        pressDownTime = Instant.now();
//                        if (listener != null) {
//                            listener.onTouched(pressDownPointF);
////                        if (lastAction == MotionEvent.ACTION_MOVE) {
////                            PointF pointF = new PointF(event.getX(), event.getY());
////                            listener.onMoveOver(pointF);
////                        } else {
...
//                        }
                    ...
                    if (event.getPointerCount() == 1 && canSingleFingerMove) {
                        //产品又要求单指也能移动
                        mCanDrag = true;
                    }
                }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (event.getPointerCount() == 1) {
                    PointF pointF = new PointF(event.getX(), event.getY());
                    if (distance(pressDownPointF, pointF) > DistanceMoved) {
                        pressDownPointF.x = -1F;
                        pressDownPointF.y = -1F;
                    }
                    if (listener != null) {
                        listener.onMove(pointF);
                    }
                }
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (!isGraffiti) {
                    ...
                    if (event.getPointerCount() == 1) {
                        ...
                        if (pressDownPointF.x > 0 && pressDownPointF.y > 0) {
                            PointF pressUpPointF = new PointF(event.getX(), event.getY());
                            //点击动作
                            long duration = Duration.between(pressDownTime, Instant.now()).getSeconds();
                            if (listener != null) {
                                if (duration < ClickTime) {
                                    float distance = distance(clickPointF, pressUpPointF);
                                    long defTime = Duration.between(lastClickTime, Instant.now()).getSeconds();
                                    if (distance <= DistanceMoved && defTime < ClickTime) {
                                        //双击
                                        listener.onDoubleClick(pressUpPointF);
                                        lastClickTime = new Instant(0);
                                    } else {
                                        clickPointF.x = pressUpPointF.x;
                                        clickPointF.y = pressUpPointF.y;

                                        lastClickTime = Instant.now();
                                        listener.onClick(pressUpPointF);
                                    }
                                }/* else {
                                listener.onLongClick(pressUpPointF);
                            }*/
                            }
...
//                                listener.onMoveOver(pointF);
        ...
        int la = event.getActionMasked();
        if (la == MotionEvent.ACTION_MOVE && pressDownPointF.x >= 0 && pressDownPointF.y >= 0) {
            //是移动
        } else {
            lastAction = la;
        }
    ...
    public void checkCenter(float centerX, float centerY, float degree) {
...
//        }
    ...
    //CaoJun
    private static int DistanceMoved = 20;
    private static double ClickTime = 0.3;
    ...
    private float[] mInitMatrixValue = new float[9]; // 动画终结时矩阵值

    private PointF pressDownPointF = new PointF(0F, 0F);
    private PointF clickPointF = new PointF(0F, 0F);
    @SuppressLint("NewApi")
    private Instant pressDownTime = Instant.now();
    ...
    public void reset() {
        ...
        mMatrix.getValues(mFromMatrixValue);/*设置矩阵动画初始值*/
        mRevertAnimator.setMatrixValue(mFromMatrixValue, mInitMatrixValue);
        mRevertAnimator.cancel();
        mRevertAnimator.start();
        mScaleFactor = 1.0f;
    ...
    private class LongClickThread extends Thread {
        ...
        public void run() {
            try {
                ...
                if (!stop && listener != null && (lastAction == MotionEvent.ACTION_DOWN || lastAction == MotionEvent.ACTION_POINTER_DOWN) && pressDownPointF.x > 0 && pressDownPointF.y > 0) {
                    listener.onLongClick(pressDownPointF);
                }
    ...
    private void stopLongClickThread() {
        if (longClickThread != null) {
            longClickThread.stop = true;
        }
    }
}
