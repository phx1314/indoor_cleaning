...
//  FrgPs
//
//  Created by 86139 on 2020-10-14 08:40:18
...
class FrgPs : BaseFrg() {
    override fun create(savedInstanceState: Bundle?) {
        setContentView(R.layout.frg_ps)
    }
    ...
    override fun disposeMsg(type: Int, obj: Any?) {
        ...
        when (type) {
            110 -> {
                ...
                if (!TextUtils.isEmpty(tv.text.toString())&&Verify.isNumeric(tv.text.toString()) && (tv.text.toString().toInt() in 1..10)) {
                    var mKeyStates = ArrayList<KeyState>()
                    if (tv.id == mTextView_1.id) {
                        mKeyStates.add(KeyState("water_motor_low", mTextView_1.text.toString()))
                    } else if (tv.id == mTextView_2.id) {
                        mKeyStates.add(KeyState("water_motor_mid", mTextView_2.text.toString()))
                    } else if (tv.id == mTextView_3.id) {
                        mKeyStates.add(KeyState("water_motor_high", mTextView_3.text.toString()))
                    }
            ...
            111 -> {//接收数据
                ...
                mGetRobotRes?.getJson()?.toMutableList()?.forEach {
                    if (it.key == "water_motor_low") {
                        mTextView_1.text = it.value
                    }
                    if (it.key == "water_motor_mid") {
                        mTextView_2.text = it.value
                    }
                    if (it.key == "water_motor_high") {
